...
<html>
	...
	<body>
		<nav class="breadcrumb">
			...
			<span class="c-gray en">&gt;</span> 问卷调查
		...
		<div class="pd-20">
			<!--会员列表 star-->
						<div class="cl pd-5 bg-1 bk-gray  mb-20">
							<span class="l">
								<a class="btn btn-primary radius" href="javascript:;" onclick="func_add_ac('发布问卷','question/addQuestion.action','800','450')">
									<i class="Hui-iconfont">&#xe600;</i> 发布问卷
								</a>
							</span>
						</div>
			<table class="table table-border table-bordered table-bg" id="table">
				<thead>
					...
					<tr class="text-c">
						<th width="40">ID</th>
						<th width="100">问卷名称</th>
						<th width="100">问卷描述</th>
						<th width="50">答题人数(点击详情)</th>
				...
				<tr class="text-c">
						...
						<td>${one.q_name }</td>
						<td>${one.q_desc }</td>
						<td><a href="javascript:void(0);" onClick="func_answer('答题情况','question/getAnswerList.action?q_id=${one.q_id}','800','450')">${one.answer_num }人</a></td>
						<td>${one.q_create_timef }</td>
						<td class="td-manage">
						    <a title="题库关联" href="javascript:;" onClick="func_add('题库关联','question/relateSurvey.action?q_id=${one.q_id}','800','450')" class="pd-5"  style="text-decoration:none" >
								<i class="Hui-iconfont">&#xe6c6;</i>题库关联
							</a>
							<a title="编辑" href="javascript:;" onClick="func_edit_ac('编辑活动','question/editQuestion.action?q_id=${one.q_id}','800','450')" class="pd-5"  style="text-decoration:none" >
								<i class="Hui-iconfont">&#xe6df;</i>编辑
							</a>
							<a class="pd-5" title="删除" href="javascript:;" onclick="one_del(this,'${one.q_id}')" class="ml-5" style="text-decoration:none">
   ...
   <script type="text/javascript">
   	//删除活动--------------------------------------------start------------------------------------------------------
   	function one_del(obj,q_id){
   	layer.confirm("确定要删除吗?",function(){  	
   	$.post("question/delQuestion.action",{q_id:q_id,q_is_del:1},function(data){
   	ajaxCallBack(data,reloadThis);
   	},"text");
   	});
   	}
   	//--------------------------------------------------end--------------------------------------------------------
   	function func_answer(title,url,w,h){
   	...
   	function func_add(title,url,w,h){
   	var index = layer.open({
			type: 2,
			title: title,
			area:['1400px','750px'],
