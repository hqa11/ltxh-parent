...
<html>
			...
			<form action="" method="post" class="form form-horizontal" id="form-articl-add">
				<div class="row cl">
					...
					<div class="formControls col-4">
						<input type="text" class="input-text" value="${user.u_phone }" placeholder="请输入用户手机" id="u_phone" name="u_phone" datatype="/^1[3,4,5,7,8]\d{9}$/" nullmsg="用户手机不能为空！" errormsg="请输入正确的手机号码">
					</div>
					<div class="col-2"></div>
				</div>
				<div class="row cl">
					...
					<div class="formControls col-4">
						<input type="text" class="input-text" value="${user.u_nick_name }" placeholder="请输入用户昵称" id="u_nick_name" name="u_nick_name"  datatype="*" nullmsg="用户昵称不能为空！">
					</div>
					<div class="col-2"></div>
				</div>
				<div class="row cl">
					...
					<div class="formControls col-4">
						<input type="text" class="input-text" value="${user.u_company_name }" placeholder="请输入公司名称" id="u_company_name" name="u_company_name"  datatype="*" nullmsg="公司名称不能为空！">
					</div>
					<div class="col-2"></div>
				</div>
				<div class="row cl">
					...
					<div class="formControls col-4">
						<input type="text" class="input-text" value="${user.u_post }" placeholder="请输入公司职称" id="u_post" name="u_post"  datatype="*" nullmsg="公司职称不能为空！">
					</div>
					<div class="col-2"></div>
				</div>
				<div class="row cl">
				...
				<div class="row cl cate-img articl-cate" id="imageDiv">
					...
					<div class="formControls col-10">
						<span class="img-upload" id="img-upload">
							<img  src="<%=basePath%>/upload/${user.u_head_img}"  img-url="${user.u_head_img}" class="thumbnail" id="artImg">
						</span>
	...
	<!-- 显示所在位置 -->
		<div class="showAndhide">
		 	<div class="row cl" id="location">
					...
					<div class="formControls col-3">
						<input type="text" class="input-text" value="${user.u_address_detail }" placeholder="请选择所在位置" id="u_address_detail" name="u_address_detail" readonly="readonly" onclick="choseAddress(this)">
					</div>
	
					<div class="col-2"></div>
				</div>
				<input type="hidden" class="u_longitude" placeholder="当前位置经度" name="u_longitude" id="u_longitude" value="${user.u_longitude}"/>
				<input type="hidden" class="u_latitude" placeholder="当前位置纬度" name="u_latitude" id="u_latitude" value="${user.u_latitude}"/>
	<!-- 显示所在位置 end-->			
				<div class="row cl" id="business_license">
					<label class="form-label col-2"><span class="c-red">*</span>组织机构代码：</label>
					<div class="formControls col-3">
						<input type="text" class="input-text" value="${user.uca_business_license}" placeholder="请输入" id="uca_business_license" name="uca_business_license"  datatype="*" nullmsg="请输入企业的组织机构代码">
					</div>
					<div class="col-2"></div>
				</div>
				<div class="row cl" id="companytel">
					...
					<div class="formControls col-3">
						<input type="text" class="input-text" value="${user.u_company_phone}"  id="u_company_phone" name="u_company_phone" placeholder="请输入企业电话">
					</div>
					<div class="col-2"></div>
				</div>
	<!-- 封面图start -->                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
				...
				<div class="row cl cate-img articl-cate" id="imageDiv2">
					...
					<div class="formControls col-10">
					   <div id="cover-box">
						...
						<c:forEach items="${user.imgs }" var="img">
						<span class="img-upload">
							<img  src="<%=basePath%>upload/${img.csi_img_path}" class="img-cover">
						</span>
						</c:forEach>
						</div>
						<span class="btn-upload">
						  <a href="javascript:void();" class="btn btn-primary radius" style="margin-left:5px;"><i class="iconfont">&#xf0020;</i> 添加图片</a>
						  <input type="file" id="input-file2" multiple name="file_0" class="input-file" onchange="return check_file2(2*1024*1024,'input-file',this)" nullimg="分享图片不能为空" errorimg="上传的附件文件不能超过2M！！！">
						  <span class="Validform_checktip"></span>
						</span>
	...
	<!-- 添加对公司的描述start -->
				<div class="row cl" id="describtion">
					...
					<div class="formControls col-10">
						<script id="editor" name="editor" datatype="editor" style="width:auto;height:400px;">${user.u_company_desc}</script>
					</div>
					<div class="col-2"></div>
				</div>  
	<!-- 添加对公司的描述end -->
	...
	<!-- 编辑页面的显示和隐藏end -->	
				...
				<div class="row cl">
					<div class="col-10 col-offset-2">
						<button onClick="updateUser();" class="btn btn-primary radius" type="button">
			...
			<!-- 用来编辑百度地图的start -->
			 <div class="baidumap"  style="display: inline-block;width:45%;height:400px;vertical-align: top;padding-top: 20px; id="showmap" >	 
		  		<div id="r-result" style="width:auto; height: 50px;font-size: 30px;">请输入:<input type="text" id="suggestId" size="20" value="${user.u_address_detail }" name="u_address_detail" style="width:200px;height:30px;" /></div>
...
$("#u_sex").val('${user.u_sex}');
$("#u_role").val('${user.u_role_id}');
 showPageDetail();
  initArea("g_place_province_id","g_place_city_id","g_place_area_id");
  //处理省市区
      $("#g_place_province_id").val('${user.u_province_id}').change();
      $("#g_place_city_id").val('${user.u_city_id}').change();
      $("#g_place_area_id").val('${user.u_area_id}');
});
//为图片添加双击事件-----------------------------------start-----------------------------------------------
...
//图片上传------------------------------------------start------------------------------------------------
	...
	function check_file2(size,id,obj){
		...
		var $img = '<span class="img-upload"><img  src="'+pic_upload_path + data+'" class="img-cover" img-url="'+data+'" ></span>';
...
params+="&u_head_img="+su_img+"&u_id=${user.u_id}"+"&u_company_desc="+text+"&csi_img_path="+corver_img.join(",");
//2.发送请求
